...
trait ProcessActionEntityFactory {
  ...
  class ProcessActionEntity(tag: Tag) extends Table[ProcessActionEntityData](tag, "process_deployment_info") {
    ...
    def deploymentAction: Rep[ActionType] = column[ActionType]("deployment_action", NotNull)
    ...
    def * : ProvenShape[ProcessActionEntityData] = (processId, processVersionId, environment, user, deployedAt, deploymentAction, commentId, buildInfo) <> (
