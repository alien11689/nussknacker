...
import pl.touk.nussknacker.restmodel.processdetails.{ActionType, ProcessAction}
...
class ProcessObjectsFinderTest extends FunSuite with Matchers with TableDrivenPropertyChecks {
  ...
  private val process1deployed = process1.copy(lastAction = Option(ProcessAction(1, LocalDateTime.now(), "user", ActionType.Deploy, Option.empty, Option.empty, Map.empty)))
